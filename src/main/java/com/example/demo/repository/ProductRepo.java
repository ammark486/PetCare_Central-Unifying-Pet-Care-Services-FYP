...
public interface ProductRepo extends JpaRepository<Product, Long> {
    ...
    @Query(value = "SELECT new com.example.demo.dto.ProductDto(p.id, p.name, p.price, p.imageUrl, p.description, p.age, p.breed, p.color, p.isActive, pt.type as productType)" +
            " FROM Product p INNER JOIN p.category c INNER JOIN c.productType pt WHERE p.category.id = :id AND p.isActive = :isActive")
//    @Query(value = "SELECT new com.example.demo.dto.ProductDto(p.id, p.name, p.price, p.imageUrl, p.description, p.age, p.breed, p.color, p.isActive)" +
//            "FROM Product p where p.category.id = :id AND p.isActive = :isActive", nativeQuery = true)
